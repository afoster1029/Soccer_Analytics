...
```{r,message=FALSE}
path = "english-premier-league/data"
file_names = dir(path,pattern=".csv")
file_1 <- file_names[1]
seasons_df <- read_csv(paste(path,file_1,sep="/")) %>%
  mutate(season = str_sub(file_1,8,11), Datetime = ymd(Date),HAGG = 0.0, AAGG=0.0, GP = 0, HTP = 0, ATP=0, HACG=0.0,AACG=0.0) %>%
  select(Datetime,Date, HomeTeam, AwayTeam, FTR,FTAG, FTHG, HTAG, HTHG, HTR,season, HAGG, AAGG,GP, ATP, HTP, HACG, AACG)


for (file in file_names) {
    if (file != file_1){
        epl_season <- (str_sub(file, 8,11))
        file_path <- paste(path,file,sep="/")
    
        df <- read_csv(file_path) %>%
      mutate(season = str_sub(file,8,11), Datetime = dmy(Date),HAGG = 0.0, AAGG=0.0, GP = 0, HTP = 0, ATP=0, HACG=0.0,AACG=0.0) %>%
      select(Datetime, Date, HomeTeam, AwayTeam, FTR,FTAG, FTHG, HTAG, HTHG, HTR,season, HAGG, AAGG,GP, ATP, HTP, HACG, AACG)

        seasons_df = rbind(seasons_df, df)
    }
}

cat('\n')
cat('Dataframe has',nrow(seasons_df),'rows')
cat('\n')
cat('Dataframe contains',length(unique(seasons_df$season)),'seasons')
```
...
* Explanatory variables: HTR, HTHG, HTAG, HAGG, AAGG, HTP, ATP, HT, AT
...
final_df <- add_explantory_var(seasons_df[which(seasons_df$season == initialize_season),]) # initialize with the teams home games from the 0910 season
...
df_train
vip(epl_randf_oob, num_features = 13, bar = FALSE)
