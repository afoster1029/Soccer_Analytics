...
```{r}
# Function takes in dataframe with all matches. Then loops through each game For every match it computes the league standings prior to the given game 
...
add_explantory_var <- function(df) {
    for (row in 1:nrow(df)){ # iterate through every game
        ...
        if (month(current_date) <  10 & month(current_date) >= 8){ # check if the game occurs at the beginning of the season
