...
cv_MLS_all_vars$results %>% 
  ...
  geom_line(aes(x=nvmax,y=RMSE - RMSESD))
```
*Using the stepwise selection method to examine which variables are most relavent for our model we can see that the best tune is 60 explanatory variables. The coefficients and the variabls are shown in the tidy table above. It's important to note that there are 50 variations of the team and opponent categorical variables because there are 26 teams total. The intercept is the Atlanta United team.*
...
```
*When using our OlS model on the testing data we have an CV RMSE of 1.3 which is slightly higher than the training CV RMSE.*
